...
/// `data/prefilter.json`과 동일한 포맷으로 직렬화/역직렬화되는 Bloom filter 구현.
/// (서버 모듈과 독립적으로 클라이언트에서만 사용)
struct BloomFilter: Codable, Hashable {
    let itemCount: Int
    ...
    init(items: [String], falsePositiveTolerance: Double = 0.001) throws {
    ...
    private mutating func insert(value: String) throws {
...
extension Data {
    ...
    func murmurHash3(seed: UInt32) -> UInt32 {
    ...
    private func block<T: FixedWidthInteger>(at index: Int) -> T {
        var block: T = 0
